...
# Resize the image array to (110, 164, 3) / 255
res = np.zeros([len(new_total_list), 110, 164, 3])
...
np.save("target_category_coding_110_164_9_6.npy", target_category_coding)
np.save("x_total_110_164_9_6.npy", res)
np.save("target_total_110_164_9_6.npy", target_total)
...
x_test = np.load("Test_110_164_9_6.npy")
# %% -------------------------------------- CNN Class ------------------------------------------------------------------
class CNN(nn.Module):
    def __init__(self):
        super(CNN, self).__init__()
...
model = CNN().to(device)
PATH = 'model_yongchaoqiaofinal.pt'
# PATH1 = 'model_yongchaoqiao9batch200.pt'
# PATH2 = 'model_yongchaoqiao9day5_2.pt'
# PATH3 = 'model_yongchaoqiao9final.pt'
# PATH4 = 'model_yongchaoqiao9weighted.pt'
model.load_state_dict(torch.load(PATH, map_location=device))
model.eval()
y_pred = model(x_test).detach().numpy()
...
# model.eval()
# y_pred3 = model(x_test).detach().numpy()
# model.load_state_dict(torch.load(PATH4, map_location=device))
# model.eval()
# y_pred4 = model(x_test).detach().numpy()
# y_pred = (y_pred4 + y_pred3) / 2
