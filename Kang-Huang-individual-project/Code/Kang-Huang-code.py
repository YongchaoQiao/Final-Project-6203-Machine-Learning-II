...
#to create a dataset that can use dataloader funtion
class PlantDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_src = os.getcwd() + '/images/' + self.df.loc[idx, 'image_id'] + '.jpg'
...
submission_df = pd.read_csv(os.getcwd() + '/sample_submission.csv')
...
for i_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df, train_y)):
    ...
    for epoch in range(N_EPOCHS):
        ...
        score = roc_auc_score(val_labels, val_preds, average='macro')

        print("Epoch {} | Train Loss {:.5f},  Valid Loss {:.5f},  Valid Score {:.5f}".format(
              epoch, tr_loss / len(dataloader_train), val_loss / len(dataloader_valid), score))
        if score > score_best:
